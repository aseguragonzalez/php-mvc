...
3. **Implement in parallel** using the code/test/docs agents under `.claude/agents/`
   (`php-mvc-implementer`, `php-mvc-test-writer`, `docs-aligner`) against those contracts.
...
- **Commits and PRs** use the bot App identity (`myclaudecodeagent[bot]`) — see the global
  ...
  reverify-commit step for the Verified badge. This step is **mandatory, not cosmetic**,
  in this repo specifically: the `main-protected` ruleset enforces `required_signatures`
  and `CONTRIBUTING.md` requires every commit to be GPG/SSH signed — a bot commit pushed
  without the reverify step is unsigned and will be rejected or fail branch protection.
  Reverifying is what lets the requester leave a genuine "Approve" review, since GitHub
  also blocks a PR author from approving their own PR.
...
`invalid`, `duplicate`, `wontfix`, `dependencies`, `github_actions`, `php`.

## Commit type → release impact

The commit type is read by `semantic-release`'s default Angular preset (no custom
`releaseRules` in `.releaserc.json`). Pick it by **which layer actually changed**, not by
...
| `src/` breaking change (renamed public class, required param added, return type changed, non-`final` → `final`, etc.) | `fix!:` / `feat!:` + `BREAKING CHANGE:` footer | major |
| `docs/`, `README.md`, `CLAUDE.md` only | `docs:` | none |
| `.github/workflows/`, CI/pipeline files | `ci:` | none |
| `Makefile`, `composer.json` build config, `.claude/` tooling, `.devcontainer/` | `chore:` / `build:` | none |
...
never `feat:`, because nothing shipped to Packagist changed.
...
4. `gh pr checks <n>` — all required checks green (`quality / Quality Check`, `Validate PR
   Title`, `codecov/patch`).
...
```bash
gh api repos/aseguragonzalez/php-mvc/pulls/<pr-number>/comments \
  -f body="<answer in English>" \
  -F in_reply_to=<review_comment_id>
```

Get `<review_comment_id>` from `gh api repos/aseguragonzalez/php-mvc/pulls/<pr-number>/comments`
